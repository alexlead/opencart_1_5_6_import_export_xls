...
//  ----------- Insert manufacturer  ---------------------------
	...
	function storeManufacturersIntoDatabase( &$database, &$products, &$manufacturerIds ) {
		...
		$sql = "REPLACE INTO `".DB_PREFIX."manufacturer` (`manufacturer_id`, `name`, `image`, `sort_order`) VALUES "; 
		$k = strlen( $sql );
		$first = TRUE;
		...
		foreach ($products as $product) {
			...
			foreach ($storeIds as $storeId) {
				if (!in_array($storeId,$storeIdsForManufacturers[$manufacturerId])) {
					...
					$sql2 = "REPLACE INTO `".DB_PREFIX."manufacturer_to_store` (`manufacturer_id`,`store_id`) VALUES ($manufacturerId,$storeId);";
					$database->query( $sql2 );
				}
			}
		}
...
// ---------------- Product Store Init ------------------
    ...
    if($this->mode_add_data==1){
		...
		foreach ($products as $product) {
			...
			$mini_Description = $database->escape($product['mini_description']);
			...
			$sql  = "REPLACE INTO `".DB_PREFIX."product` (`product_id`,`quantity`,`sku`,`upc`,`ean`,`jan`,`isbn`,`mpn`,`location`,";
			...
			$sql2 = "REPLACE INTO `".DB_PREFIX."product_description` (`product_id`,`language_id`,`name`,`description`,`mini_description`,`meta_description`,`meta_keyword`,`tag`) VALUES ";
			$sql2 .= "($productId,$languageId,'$productName','$productDescription','$mini_Description','$meta_description','$meta_keywords','$tags');";
			$database->query($sql);
			$database->query($sql2);
        ...
        // --- in case string        
             ...
             if($this->mode_add_data!=1){
                $sql4 = "DELETE FROM `".DB_PREFIX."url_alias` WHERE `query` = 'product_id=$productId';";
                $database->query($sql4);
                 }
			...
			foreach ($storeIds as $storeId) {
				if (in_array((int)$storeId,$availableStoreIds)) {
					$sql6 = "REPLACE INTO `".DB_PREFIX."product_to_store` (`product_id`,`store_id`) VALUES ($productId,$storeId);";
					$database->query($sql6);
				}
			}
			$layouts = array();
			...
			foreach ($layouts as $storeId => $layoutId) {
				$sql7 = "REPLACE INTO `".DB_PREFIX."product_to_layout` (`product_id`,`store_id`,`layout_id`) VALUES ($productId,$storeId,$layoutId);";
				$database->query($sql7);
			}
    ...
    // ---- related products 
         ...
         if($this->mode_add_data!=1){
            $database->query("DELETE FROM `".DB_PREFIX."product_related` WHERE `product_id` IN ($id_for_delete);");
        }
...
// ------------------------ UPLOAD PRODUCTs ----------------------
	function uploadProducts( &$reader, &$database ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$mainCategory = $this->getCell($data,$i,$j++);
			...
			$mini_description = $this->getCell($data,$i,$j++);
			$mini_description = htmlentities( $mini_description, ENT_QUOTES, $this->detect_encoding($mini_description) );
...
// ---------------------- Categories Store Init  --------------------- 
	function storeCategoriesIntoDatabase( &$database, &$categories ) 
	{
		...
		foreach ($categories as $category) {
			...
			$status = ((strtoupper($status)=="TRUE") || (strtoupper($status)=="YES") || (strtoupper($status)=="ENABLED")) ? 1 : 0;
			$sql2 = "REPLACE INTO `".DB_PREFIX."category` (`category_id`, `image`, `parent_id`, `top`, `column`, `sort_order`, `date_added`, `date_modified`, `status`) VALUES ";
			...
			$sql2 .= " $status);";
			$database->query( $sql2 );
			$sql3 = "REPLACE INTO `".DB_PREFIX."category_description` (`category_id`, `language_id`, `name`, `description`, `meta_description`, `meta_keyword`) VALUES ";
          ...
          if($this->mode_add_data!=1){
                $sql5 = "DELETE FROM `".DB_PREFIX."url_alias` WHERE `query` = 'category_id=$categoryId';";
                $database->query($sql5);
                 }
...
// ------ add category path 
            $this->db->query("DELETE FROM `" . DB_PREFIX . "category_path` WHERE category_id = " . (int)$categoryId );
		          ...
		          $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "category_path` WHERE category_id = '" . (int)$parentId . "' ORDER BY `level` ASC");
		              ...
		              foreach ($query->rows as $result) {
			                 $this->db->query("REPLACE INTO `" . DB_PREFIX . "category_path` SET `category_id` = '" . (int)$categoryId . "', `path_id` = '" . (int)$result['path_id'] . "', `level` = '" . (int)$level . "'");
			...
			foreach ($storeIds as $storeId) {
				if (in_array((int)$storeId,$availableStoreIds)) {
					$sql6 = "REPLACE INTO `".DB_PREFIX."category_to_store` (`category_id`,`store_id`) VALUES ($categoryId,$storeId);";
					$database->query($sql6);
				}
			}
			...
			foreach ($layouts as $storeId => $layoutId) {
				$sql7 = "REPLACE INTO `".DB_PREFIX."category_to_layout` (`category_id`,`store_id`,`layout_id`) VALUES ($categoryId,$storeId,$layoutId);";
				$database->query($sql7);
			}
		}
    ...
    if($this->mode_add_data==1){
		...
		foreach ($options as $option) {
			...
			if (!isset($optionIds[$name][$type])) {
				...
				$sql = "REPLACE INTO `".DB_PREFIX."option` (`option_id`,`type`,`sort_order`) VALUES ($optionId,'$type',$maxOptionSortOrder);";
				$database->query( $sql );
				$sql = "REPLACE INTO `".DB_PREFIX."option_description` (`option_id`,`language_id`,`name`) VALUES ($optionId,$langId,'".$database->escape($name)."');";
				$database->query( $sql);
			}
			if (($type=='select') || ($type=='checkbox') || ($type=='radio') || ($type=='image')) {
				...
				if (!isset($optionValueIds[$name][$type][$value][$image])) {
					...
					$sql  = "REPLACE INTO `".DB_PREFIX."option_value` (`option_value_id`,`option_id`,`image`,`sort_order`) VALUES ";
					$sql .= "($optionValueId,$optionId,'".$database->escape($image)."',$sortOrder);";
					$database->query( $sql );
					$sql  = "REPLACE INTO `".DB_PREFIX."option_value_description` (`option_value_id`,`language_id`,`option_id`,`name`) VALUES ";
			...
			if (!isset($productOptionIds[$productId][$optionId])) {
				...
				$sql  = "REPLACE INTO `".DB_PREFIX."product_option` (`product_option_id`,`product_id`,`option_id`,`option_value`,`required`) VALUES ";
			...
			if (($type=='select') || ($type=='checkbox') || ($type=='radio') || ($type=='image')) {
				...
				$sql  = "REPLACE INTO `".DB_PREFIX."product_option_value` (`product_option_value_id`,`product_option_id`,`product_id`,`option_id`,`option_value_id`,`quantity`,`subtract`,`price`,`price_prefix`,`points`,`points_prefix`,`weight`,`weight_prefix`) VALUES ";
    ...
    if($this->mode_add_data==1){
		...
		foreach ($attributes as $attribute) {
			...
			if (!isset($attributeGroupIds[$group])) {
				...
				$attributeGroupIds[$group] = $attributeGroupId;
				$sql  = "REPLACE INTO `".DB_PREFIX."attribute_group` (`attribute_group_id`,`sort_order`) VALUES ";
				$sql .= "($attributeGroupId,$maxAttributeGroupSortOrder);";
				$database->query( $sql );
				$sql  = "REPLACE INTO `".DB_PREFIX."attribute_group_description` (`attribute_group_id`,`language_id`,`name`) VALUES ";
			...
			if (!isset($attributeIds[$group][$name])) {
				...
				$sql  = "REPLACE INTO `".DB_PREFIX."attribute` (`attribute_id`,`attribute_group_id`,`sort_order`) VALUES ";
				$sql .= "($attributeId,$attributeGroupId,$attributeSortOrder);";
				$database->query( $sql );
				$sql  = "REPLACE INTO `".DB_PREFIX."attribute_description` (`attribute_id`,`language_id`,`name`) VALUES ";
				$sql .= "($attributeId,$langId,'".$database->escape($name)."');";
				$database->query( $sql );
			}
			$attributeId = $attributeIds[$group][$name];
			$sql  = "REPLACE INTO `".DB_PREFIX."product_attribute` (`product_id`,`attribute_id`,`language_id`,`text`) VALUES ";
    ...
    if($this->mode_add_data==1){
		...
		foreach ($specials as $special) {
			...
			if (!isset($customerGroups[$name])) {
				...
				$sql  = "REPLACE INTO `".DB_PREFIX."customer_group` (`customer_group_id`, `approval`, `company_id_display`, `company_id_required`, `tax_id_display`, `tax_id_required`, `sort_order`) VALUES "; 
				$sql .= "($maxCustomerGroupId, 0, 1, 0, 0, 1, 1)";
				$sql .= ";\n";
				$database->query($sql);
				$sql  = "REPLACE INTO `".DB_PREFIX."customer_group_description` (`customer_group_id`, `language_id`, `name`, `description`) VALUES ";
		...
		$productSpecialId = 0;
		$first = TRUE;
		$sql = "REPLACE INTO `".DB_PREFIX."product_special` (`product_special_id`,`product_id`,`customer_group_id`,`priority`,`price`,`date_start`,`date_end` ) VALUES "; 
		foreach ($specials as $special) {
			$productSpecialId += 1;
    ...
    if($this->mode_add_data==1){
		...
		$first = TRUE;
		$sql = "REPLACE INTO `".DB_PREFIX."product_discount` (`product_discount_id`,`product_id`,`customer_group_id`,`quantity`,`priority`,`price`,`date_start`,`date_end` ) VALUES "; 
		foreach ($discounts as $discount) {
			$productDiscountId += 1;
            ...
            $id_for_delete .= ($first) ? "" : ", ";
            $id_for_delete .= $discount['product_id'];
        ...
        if($this->mode_add_data!=1){
            $database->query("DELETE FROM `".DB_PREFIX."product_discount` WHERE `product_id` IN ($id_for_delete);");
        }
    ...
    if($this->mode_add_data==1){
		...
		$first = TRUE;
		$sql = "REPLACE INTO `".DB_PREFIX."product_reward` (`product_reward_id`,`product_id`,`customer_group_id`,`points` ) VALUES "; 
		foreach ($rewards as $reward) {
			$productRewardId += 1;
            ...
            $id_for_delete .= ($first) ? "" : ", ";
            $id_for_delete .= $reward['product_id'];
  ...
  if($this->mode_add_data!=1){
            $database->query("DELETE FROM `".DB_PREFIX."product_reward` WHERE `product_id` IN ($id_for_delete);");
        }
...
// ------------------- insert data for additional images ------------------
    ...
    if($this->mode_add_data==1){
		...
		foreach ($images as $image) {
            ...
            $id_for_delete .= ($first) ? "" : ", ";
            $id_for_delete .= $image['product_id'];
        ...
        if($this->mode_add_data!=1){
            $database->query("DELETE FROM `".DB_PREFIX."product_image` WHERE `product_id` IN ($id_for_delete);");
        }
...
// ----------- TABLES VALIDATOR ---------------
	...
	function validateProducts( &$reader )
	{
		...
		( "product_id", "name", "categories", "main_categories", "sku", "upc", "ean", "jan", "isbn", "mpn", "location", "quantity", "model", "manufacturer", "image_name", "requires\nshipping", "price", "points", "date_added", "date_modified", "date_available", "weight", "unit", "length", "width", "height", "length\nunit", "status\nenabled", "tax_class_id", "viewed", "language_id", "seo_keyword", "description","mini_description", "meta_description", "meta_keywords", "stock_status_id", "store_ids", "layout", "related_ids", "tags", "sort_order", "subtract", "minimum" );
		$data =& $reader->getSheet(1);
...
// --------- FILE VALIDATOR for correct format ----------------
	function validateUpload( &$reader )
	{
		$sheets = $reader->getSheetCount();
        
        if ($sheets != 8 && $this->switch_file_type==1) {
			error_log(date('Y-m-d H:i:s - ', time()).$this->language->get( 'error_sheet_count' )."\n",3,DIR_LOGS."error.txt");
			return FALSE;
		} elseif ($sheets != 1 && $this->switch_file_type==2){
			error_log(date('Y-m-d H:i:s - ', time()).$this->language->get( 'error_sheet_count' )."\n",3,DIR_LOGS."error.txt");
			return FALSE;            
        } elseif ($sheets != 7 && $this->switch_file_type==3){
			error_log(date('Y-m-d H:i:s - ', time()).$this->language->get( 'error_sheet_count' )."\n",3,DIR_LOGS."error.txt");
			return FALSE;
        }
        
    if($this->switch_file_type==1||$this->switch_file_type==2){
    ...
    if($this->switch_file_type==1||$this->switch_file_type==3){
...
// ------------- IMPORT DATA INIT  --------------------
	...
	function upload( $filename, $mode_add_data = 1,  $switch_file_type = 1) {
        ...
        $this->mode_add_data = $mode_add_data;
		$this->switch_file_type = $switch_file_type;
...
/*  -----   Export data from DB  ------  */
	...
	function getMainCategoriesForProducts( &$database ) {
		...
		$sql =  "SELECT * FROM `".DB_PREFIX."product_to_category` WHERE `category_id` = 1";
		$result = $database->query( $sql );
		foreach ($result->rows as $row) {	
	...
	function populateProductsWorksheet( &$worksheet, &$database, $languageId, &$priceFormat, &$boxFormat, &$weightFormat, &$textFormat )
	{
		...
		$worksheet->setColumn($j,$j++,max(strlen('mini_description'),32)+1,$textFormat);
		...
		$worksheet->writeString( $i, $j++, 'seo_keyword', $boxFormat );
		$worksheet->writeString( $i, $j++, 'description', $boxFormat );
		$worksheet->writeString( $i, $j++, 'mini_description', $boxFormat );
		$worksheet->writeString( $i, $j++, 'meta_description', $boxFormat );
		$worksheet->writeString( $i, $j++, 'meta_keywords', $boxFormat );
		...
		foreach ($result->rows as $row) {
            ...
            $worksheet->writeString( $i, $j++, html_entity_decode($row['mini_description'],ENT_QUOTES,'UTF-8'), $textFormat, TRUE );
    ...
    if(isset($finish_id))$this->finish_id = $finish_id;
		...
		$file_name = 'backup_categories_products.xls';
        if($act==1) $file_name = 'backup_categories.xls';
        if($act==2) $file_name = 'backup_products.xls';
